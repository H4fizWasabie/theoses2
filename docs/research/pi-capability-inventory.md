...
| Project trust | `coding-agent/src/core/project-trust.ts`, `core/trust-manager.ts` | Per-directory trust prompt/decision store (`~/.pi/agent/trust.json`) gating whether `.pi/settings.json`, project extensions, and project packages load; walks up parent directories. | Directly modeled around "project = a directory on disk," parallel to a git-repo trust model (e.g., VS Code workspace trust). |
| Skills (product layer) | `coding-agent/src/core/skills.ts` | Product-level skill loading/invocation (`/skill:name`) built on `pi-agent-core`'s skill loader, with additional directory search paths (`.agents/skills`, `.pi/skills`). | Filesystem-directory based, same as agent-core. |
| Prompt templates | `coding-agent/src/core/prompt-templates.ts` | Loads Markdown prompt templates from `~/.pi/agent/prompts/` or `.pi/prompts/`, expandable via `/name` with `{{variable}}` substitution. | Filesystem-based; not coding-specific in concept. |
| Model runtime, resolver, registry, config | `coding-agent/src/core/model-runtime.ts` (28.5K), `core/model-resolver.ts` (25.7K), `core/model-registry.ts`, `core/model-config.ts`, `core/models-store.ts`, `core/remote-catalog-provider.ts` | Resolves and caches available models/providers from `pi-ai`, handles scoped-model cycling (`/scoped-models`, Ctrl+P), custom provider config via `~/.pi/agent/models.json`. | None specific to coding; generic LLM provider/model management. |
...
| Settings & keybindings | `coding-agent/src/core/settings-manager.ts` (42K), `core/keybindings.ts` (12K), `core/resolve-config-value.ts`, `core/settings-diagnostics.ts` | Layered global (`~/.pi/agent/settings.json`) + project (`.pi/settings.json`) settings; fully remappable keybindings (`~/.pi/agent/keybindings.json`). | Project-layer settings assume a project directory; keybindings are terminal-input-specific. |
| Context files (AGENTS.md/CLAUDE.md loading) | `coding-agent/src/core/resource-loader.ts` (39K) | Loads and concatenates `AGENTS.md`/`CLAUDE.md`/`AGENTS.override.md` from global, parent-directory, and cwd locations into the system prompt; supports `.pi/SYSTEM.md` overrides. | Directory-tree walk from cwd upward — assumes a nested project directory structure (git-repo-like). |
...
| Eval CLI runner | `evals/scripts/run-evals.mjs` (referenced by `npm run eval`) | Wires `--provider`/`--model` CLI args (or `PI_PROVIDER`/`PI_MODEL` env vars) through Pi's normal `ModelRuntime` auth (subscription credentials or API keys) into the eval run. | None beyond reusing coding-agent's auth/model resolution. |
