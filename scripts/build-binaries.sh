#!/usr/bin/env bash
#
# Build theoses binaries for all platforms locally.
...
#     theoses-darwin-arm64.tar.gz
#     theoses-darwin-x64.tar.gz
#     theoses-linux-x64.tar.gz
#     theoses-linux-arm64.tar.gz
#     theoses-windows-x64.zip
#     theoses-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before theoses starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/theoses.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/theoses"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating theoses-$platform.zip..."
        (cd "$platform" && zip -r ../theoses-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating theoses-$platform.tar.gz..."
        mv "$platform" theoses && tar -czf theoses-$platform.tar.gz theoses && mv theoses "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../theoses-$platform.zip)
    else
        tar -xzf theoses-$platform.tar.gz && mv theoses "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/theoses.exe"
    else
        echo "  $OUTPUT_DIR/$platform/theoses"
    fi
done
