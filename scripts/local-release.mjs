...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "theoses-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `theoses-${platform}.zip` : `theoses-${platform}.tar.gz`;
...
function createPiShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "theoses.cmd"))) {
			writeFileSync(join(installDirectory, "theoses.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\theoses.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "theoses.ps1"), '& "$PSScriptRoot/node_modules/.bin/theoses.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "theoses.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\theoses.exe" %*\r\n');
		writeFileSync(join(installDirectory, "theoses.ps1"), '& "$PSScriptRoot/node_modules/.bin/theoses.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "theoses"), join(installDirectory, "theoses"));
}
...
if (rootPackageJson.name !== "theoses-monorepo") {
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `theoses-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "theoses.exe" : "theoses")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "theoses.cmd" : "theoses")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "theoses.cmd" : "theoses")} --help`);
	}
}
