...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	const root = await mkdtemp(join(tmpdir(), "theoses-sync-versions-"));
	try {
		...
		await writeManifest(root, "packages/evals", {
			...
			private: true,
			dependencies: {
				"theoses-coding-agent": "^1.0.0",
				"some-other-scope/theoses-ai": "npm:theoses-ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["theoses-coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["some-other-scope/theoses-ai"], "npm:theoses-ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["theoses-coding-agent"], "^1.0.0");
