...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("theoses", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn theoses: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".theoses/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn theoses subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".theoses/agent/AGENTS.md");
