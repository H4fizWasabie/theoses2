...
Theoses and also where the boundaries are.

In general Theoses is a coding agent that runs locally within the security boundary
...
Theoses treats the local user account and files writable by that account as inside
the same trust boundary as the Theoses process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Theoses configuration, they can generally influence Theoses or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Theoses grants that write access or crosses
an operating-system privilege boundary.

Theoses relies on users installing trustworthy extensions and loading trustworthy
skills and only to use theoses within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in theoses or another package in
...
- Local code execution or sandboxing behavior (the Theoses coding agent intentionally does not have a sandbox)
- Behavior of theoses extensions or skills installed by the user
...
- Public internet exposure of a Theoses installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.theoses`,
  `~/.theoses/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Theoses itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the theoses coding agent.
...
For example, a report showing that malicious contents written to a trusted Theoses
configuration file cause Theoses to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Theoses.  For exposed-secret reports,
include evidence that the credential is owned by the project or grants access to
project-operated infrastructure or services.
