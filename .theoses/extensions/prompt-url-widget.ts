...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "theoses-coding-agent";
import { Container, hyperlink, Text } from "theoses-tui";
...
async function fetchAdvisoryMetadata(theoses: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await theoses.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	theoses: ExtensionAPI,
	kind: PromptMatch["kind"],
	target: string,
	cwd: string,
): Promise<GhMetadata | undefined> {
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(theoses, cwd, target);
	}
	...
	try {
		const result = await theoses.exec("gh", args);
...
export default function promptUrlWidgetExtension(theoses: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = theoses.getSessionName()?.trim();
		if (!currentName) {
			theoses.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(theoses, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	theoses.on("session_switch", async (_event, ctx) => {
