/**
 * Import a Theoses session shared as a gist by the issue-analysis CI workflow
 ...
 *   /ir https://github.com/H4fizWasabie/theoses2/issues/123
 *
 *   theoses "/ir <gist-id>"
 */
...
import type { ExtensionAPI, ExtensionCommandContext } from "theoses-coding-agent";
...
function decodeExportedHtml(html: string): { header: SessionHeader; jsonl: string } {
	...
	if (!match) throw new Error("HTML does not contain embedded Theoses session data");

	let data: unknown;
	try {
		data = JSON.parse(Buffer.from(match[1], "base64").toString("utf8"));
	} catch {
		throw new Error("embedded Theoses session data is not valid JSON");
	}
	...
	if (!header || header.type !== "session" || typeof header.id !== "string" || typeof header.cwd !== "string") {
		throw new Error("embedded Theoses session data has no valid session header");
	}
	if (!Array.isArray(sessionData.entries)) {
		throw new Error("embedded Theoses session data has no entries array");
	}
...
function getCiWorkdirName(sourceCwd: string): string | undefined {
	...
	return /^theoses-ci-[0-9a-f]{32}$/i.test(name) ? name : undefined;
}
