...
describe("NodeExecutionEnv", () => {
	...
	it.each([
		...
		["an empty override shadows the base value", { THEOSES_SESSION_FILE: "" }, "x:"],
		[
		...
		async (_description, overrides, expectedSessionFile) => {
			...
			const env = new NodeExecutionEnv({
				cwd: root,
				shellEnv: {
					THEOSES_SESSION_FILE: "/stale/parent.jsonl",
					THEOSES_CODING_AGENT: "true",
					THEOSES_NODE_ENV_PRESERVED_TEST: "preserved",
				},
			});
			const result = getOrThrow(
				await env.exec(
					`printf '%s:%s|%s|%s' "\${THEOSES_SESSION_FILE+x}" "\${THEOSES_SESSION_FILE-}" "$THEOSES_CODING_AGENT" "$THEOSES_NODE_ENV_PRESERVED_TEST"`,
					{ env: overrides },
				),
			);
	...
	it("can replace rather than inherit the default shell environment", async () => {
		const root = createTempDir();
		const inheritedKey = "THEOSES_NODE_ENV_INHERITED_TEST";
		const configuredKey = "THEOSES_NODE_ENV_CONFIGURED_TEST";
		const explicitKey = "THEOSES_NODE_ENV_EXPLICIT_TEST";
