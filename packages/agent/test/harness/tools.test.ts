	...
	describe("bash", () => {
		...
		it("prepares command, cwd, and an explicit environment with the turn context", async () => {
			const env = new NodeExecutionEnv({
				cwd: createTempDir(),
				shellEnv: { THEOSES_BASH_PREPARE_INHERITED: "inherited" },
			});
			...
			const tool = createBashTool<typeof context>({
				...
				prepare: async (execution, turnContext, signal) => {
					...
					execution.env = { THEOSES_BASH_PREPARE_EXPLICIT: "explicit" };
					execution.inheritEnv = false;
					execution.command += `\nprintf '%s:%s:%s:%s' "$prefix" "\${THEOSES_BASH_PREPARE_INHERITED-}" "$THEOSES_BASH_PREPARE_EXPLICIT" "$PWD"`;
				},
			});
