...
export const AI_TELEMETRY_SCHEMA = {
	...
	spans: {
		"theoses.ai.request": {
			...
			endAttributes: {
				"theoses.ai.response.model": { type: "string", description: "Concrete response model" },
				"theoses.ai.response.id": {
					type: "string",
					cardinality: "high",
					description: "Provider response id",
				},
				"theoses.ai.response.stop_reason": {
					type: "string",
					values: ["stop", "length", "tool_use", "error", "aborted", "deferred"],
					description: "Normalized terminal response reason",
				},
				"theoses.ai.http.status_code": { type: "number", description: "Final HTTP status" },
				"theoses.ai.usage.input_tokens": { type: "number", description: "Reported input tokens" },
				"theoses.ai.usage.output_tokens": { type: "number", description: "Reported output tokens" },
				"theoses.ai.usage.cache_read_tokens": { type: "number", description: "Reported cache-read tokens" },
				"theoses.ai.usage.cache_write_tokens": {
					type: "number",
					description: "Reported cache-write tokens",
				},
				"theoses.ai.usage.reasoning_tokens": { type: "number", description: "Reported reasoning tokens" },
				"theoses.ai.usage.total_tokens": { type: "number", description: "Reported total tokens" },
				"theoses.ai.usage.cost": { type: "number", description: "Reported total cost" },
				"theoses.ai.stream.chunk_count": { type: "number", description: "Streamed update chunk count" },
				"theoses.ai.stream.time_to_first_chunk_ms": {
					type: "number",
					description: "Elapsed milliseconds to first update chunk",
				},
				"theoses.ai.error.type": {
					type: "string",
					cardinality: "low",
...
const operationStartAttributes = {
	...
	"theoses.operation.id": {
		type: "string",
		required: true,
		cardinality: "high",
		description: "Durable operation id",
	},
	"theoses.operation.recovery": {
		type: "boolean",
		required: true,
...
export const HARNESS_TELEMETRY_SCHEMA = {
	...
	spans: {
		...
		"theoses.harness.checkpoint": {
			...
			parents: { kind: "spans", spans: ["theoses.harness.run"] },
			startAttributes: {
				"theoses.lane.name": {
					type: "string",
					required: true,
		...
		"theoses.harness.step": {
			...
			startAttributes: {
				...
				"theoses.step.attempt": {
					type: "number",
					required: true,
					description: "One-based durable attempt number",
				},
				"theoses.compaction.reason": {
					type: "string",
					required: false,
		...
		"theoses.harness.tool": {
			...
			parents: { kind: "spans", spans: ["theoses.harness.turn", "theoses.harness.run"] },
			startAttributes: {
				"theoses.lane.name": {
					type: "string",
					required: true,
		...
		"theoses.harness.sleep": {
			...
			parents: { kind: "spans", spans: ["theoses.harness.step", "theoses.harness.run"] },
			startAttributes: {
				"theoses.operation.id": {
					type: "string",
					required: true,
