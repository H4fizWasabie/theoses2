...
function getActiveManagedInstallRoot(): string | undefined {
	const configuredRoot = process.env.THEOSES_MANAGED_INSTALL_ROOT?.trim();
...
async function runManagedSelfUpdate(managedRoot: string, version: string): Promise<void> {
	...
	try {
		cleanupManagedStaging(managedRoot);
		const installerApiBase = (process.env.THEOSES_INSTALLER_API_BASE?.trim() || DEFAULT_INSTALLER_API_BASE).replace(
			/\/+$/,
			"",
		);
