...
export interface CreateCodingAgentHarnessOptions extends Omit<AgentHarnessOptions, "toolContext" | "tools"> {
	...
	/** Path to the JSONL session file exposed to default bash commands as THEOSES_SESSION_FILE. */
	sessionFile?: string;
			...
			createCodingAgentHarnessTool(
				createBashTool<ExecutionToolContext>({
					...
					prepare: async (execution) => {
						...
						execution.env.THEOSES_SESSION_ID = metadata.id;
						execution.env.THEOSES_SESSION_FILE = sessionFile ?? "";
						execution.env.THEOSES_PROVIDER = model.provider;
						execution.env.THEOSES_MODEL = model.id;
						execution.env.THEOSES_REASONING_LEVEL = thinkingLevel;
					},
				}),
				toolContext,
				{
