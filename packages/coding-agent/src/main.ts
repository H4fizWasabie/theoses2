...
import { type ImageContent, modelsAreEqual } from "theoses-ai";
...
export async function main(args: string[], options?: MainOptions) {
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.THEOSES_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: THEOSES_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
