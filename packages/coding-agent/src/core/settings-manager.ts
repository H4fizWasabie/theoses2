...
import type { Transport } from "theoses-ai";
import type { TuiMode as RendererTuiMode, ScrollViewScrollbar } from "theoses-tui";
...
export interface Settings {
	...
	httpProxy?: string; // Proxy URL applied as HTTP_PROXY and HTTPS_PROXY for Theoses-managed HTTP clients
...
export class SettingsManager {
	...
	getClearOnShrink(): boolean {
		...
		return process.env.THEOSES_CLEAR_ON_SHRINK === "1";
	}
	...
	getShowHardwareCursor(): boolean {
		return this.settings.showHardwareCursor ?? process.env.THEOSES_HARDWARE_CURSOR === "1";
	}
