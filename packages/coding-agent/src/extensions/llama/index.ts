...
export default function llamaExtension(theoses: ExtensionAPI): void {
	const provider = createLlamaProvider();
	theoses.registerProvider(provider.provider);
	...
	const syncCatalog = async (
		...
		const result = await ctx.modelRegistry.refresh({
			...
			// /llama already contacted the configured llama.cpp server, so keep this refresh live even in THEOSES_OFFLINE.
			allowNetwork: true,
			signal,
		});
