...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".theoses", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".theoses", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".theoses", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".theoses"), { recursive: true });
			writeFileSync(join(cwd, ".theoses", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".theoses"), { recursive: true, force: true });
