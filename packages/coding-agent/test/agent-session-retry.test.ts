...
import { Agent, type AgentEvent, type AgentTool } from "theoses-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "theoses-ai/compat";
...
describe("AgentSession retry", () => {
	...
	it("retries a garbled tool-call turn reported as generic finish_reason error (#341)", async () => {
		const created = await createSession({ failCount: 0 });
		let callCount = 0;
		const streamFn = () => {
			callCount++;
