import { Container } from "theoses-tui";
...
describe("issue #7829 invalid settings warning", () => {
	...
	it("renders startup diagnostics inside the transcript", async () => {
		...
		const previousOffline = process.env.THEOSES_OFFLINE;
		...
		try {
			...
			if (previousOffline === undefined) delete process.env.THEOSES_OFFLINE;
			else process.env.THEOSES_OFFLINE = previousOffline;
			harness.cleanup();
		}
	});
});
