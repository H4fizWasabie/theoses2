...
describe("extensions discovery", () => {
	...
	it("loads the coding-agent entrypoint without rewriting pi-ai provider subpaths", async () => {
		fs.writeFileSync(
			...
			`
				import { getAgentDir } from "theoses-coding-agent";
				void getAgentDir;
				export default function(pi) {
					pi.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only pi-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			...
			`
				import * as oauth from "theoses-ai/oauth";
				void oauth;
				export default function(pi) {
					pi.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json theoses field", async () => {
	...
	it("package.json with theoses field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without theoses field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ theoses: { extensions: ["./entry.ts"] } }));
