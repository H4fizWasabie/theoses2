...
import { Text, type TUI } from "theoses-tui";
...
describe("ToolExecutionComponent parity", () => {
	...
	test("bash renderer does not duplicate final full output truncation details", async () => {
		...
		expect(rendered).toContain("Truncated: showing 500 of 4000 lines");
		expect(rendered).not.toContain("[Showing lines 3501-4000 of 4000. Full output:");
	});
	...
	for (const scenario of [
		...
		{
			title: "AGENTS.md",
			path: join(process.cwd(), ".theoses", "AGENTS.md"),
			content: "Hidden resource instructions",
			compact: "read resource .theoses/AGENTS.md",
			hidden: "Hidden resource instructions",
			absent: undefined,
		},
		{
			title: "AGENTS.override.md",
			path: join(process.cwd(), ".theoses", "AGENTS.override.md"),
			content: "Hidden override instructions",
			compact: "read resource .theoses/AGENTS.override.md",
