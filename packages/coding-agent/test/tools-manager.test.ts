...
const originalOffline = process.env.THEOSES_OFFLINE;
...
afterEach(() => {
	if (originalOffline === undefined) delete process.env.THEOSES_OFFLINE;
	else process.env.THEOSES_OFFLINE = originalOffline;
});
