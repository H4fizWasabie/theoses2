...
describe("AgentSession dynamic tool registration", () => {
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(sessionEnv).toMatchObject({
			THEOSES_SESSION_ID: session.sessionId,
			THEOSES_SESSION_FILE: session.sessionFile,
			THEOSES_PROVIDER: model.provider,
			THEOSES_MODEL: model.id,
			THEOSES_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("THEOSES_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("THEOSES_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("THEOSES_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("THEOSES_MODEL");
		expect(optedOutEnv).not.toHaveProperty("THEOSES_REASONING_LEVEL");
