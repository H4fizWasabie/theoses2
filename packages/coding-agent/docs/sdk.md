...
```bash
npm install theoses-coding-agent
```
...
The `Agent` class (from `theoses-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
```typescript
import { getModel } from "theoses-ai";
import { ModelRuntime } from "theoses-coding-agent";
...
import { createAgentSession, defineTool } from "theoses-coding-agent";
...
```typescript
import type { InlineExtension } from "theoses-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "theoses-coding-agent";
