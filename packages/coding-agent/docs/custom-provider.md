...
Extensions can register custom model providers via `theoses.registerProvider()`. This enables:
...
Extensions can register either a complete theoses-ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. Theoses composes `models.json` overrides above registered native providers.
...
export default function (theoses: ExtensionAPI) {
  theoses.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  theoses.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. theoses waits for the factory before startup continues, so the provider is available during interactive startup and to `theoses --list-models`.
...
theoses.registerProvider("openai", {
  headers: {
...
theoses.registerProvider("google", {
...
```typescript
theoses.registerProvider("my-llm", {
...
Use `theoses.unregisterProvider(name)` to remove a provider that was previously registered via `theoses.registerProvider(name, ...)`:
...
// Later, remove it
theoses.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map theoses levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
theoses.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "theoses-ai";
...
Credentials are persisted in `~/.theoses/agent/auth.json`:
...
- [anthropic.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, theoses can recover automatically by compacting the conversation and retrying. This recovery only kicks in if theoses recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of theoses's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/H4fizWasabie/theoses2/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message theoses does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase theoses recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
export default function (theoses: ExtensionAPI) {
  ...
  theoses.on("message_end", (event, ctx) => {
...
`message_end` runs before theoses tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what theoses checks. With this in place, theoses will:
...
- Match a provider-specific pattern, not theoses's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of theoses's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/H4fizWasabie/theoses2/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps theoses thinking levels to provider/model-specific values; null marks a level unsupported. */
