> theoses can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend theoses's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.theoses/agent/extensions/` (global) or `.theoses/extensions/` (project-local) for auto-discovery. Use `theoses -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `theoses.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `theoses.registerCommand()`
- **Session persistence** - Store state that survives restarts via `theoses.appendEntry()`
...
export default function (theoses: ExtensionAPI) {
  // React to events
  theoses.on("session_start", async (_event, ctx) => {
...
| Package | Purpose |
|---------|---------|
| `theoses-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `theoses-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `theoses-tui` | TUI components for custom rendering |
...
export default function (theoses: ExtensionAPI) {
  // Subscribe to events
  theoses.on("event_name", async (event, ctx) => {
  ...
  theoses.registerTool({ ... });
  theoses.registerCommand("name", { ... });
  theoses.registerShortcut("ctrl+x", { ... });
  theoses.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, theoses awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `theoses.registerProvider()` are flushed.
...
export default async function (theoses: ExtensionAPI) {
  ...
  theoses.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `theoses --list-models`.
...
/name or theoses.setSessionName()
...
thinking level changes (settings, keybinding, theoses.setThinkingLevel())
...
Fired before theoses decides whether to trust a project with dynamic configs (`.theoses` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
theoses.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether theoses asks, trusts, or declines by default.
...
```typescript
theoses.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
theoses.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `theoses.setSessionName()`.

```typescript
theoses.on("session_info_changed", async (event, ctx) => {
...
```typescript
theoses.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, theoses emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
theoses.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, theoses emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
theoses.on("session_compact", async (event, ctx) => {
...
theoses.on("session_compact_failed", async (event, ctx) => {
  // event.reason - "manual" (/compact), "threshold", or "overflow"
...
```typescript
theoses.on("session_before_tree", async (event, ctx) => {
...
theoses.on("session_tree", async (event, ctx) => {
...
```typescript
theoses.on("session_shutdown", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Theoses uses to build the system prompt. This lets you inspect what Theoses has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later `before_agent_start` handlers can still modify it again.

#### agent_start / agent_end / agent_settled

`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but Theoses may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know Theoses will not continue running automatically.

```typescript
theoses.on("agent_start", async (_event, ctx) => {});

theoses.on("agent_end", async (event, ctx) => {
...
```typescript
theoses.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

theoses.on("turn_end", async (event, ctx) => {
...
```typescript
theoses.on("message_start", async (event, ctx) => {
  // event.message
});

theoses.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

theoses.on("message_end", async (event, ctx) => {
...
```typescript
theoses.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

theoses.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

theoses.on("tool_execution_end", async (event, ctx) => {
...
```typescript
theoses.on("context", async (event, ctx) => {
...
```typescript
theoses.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header theoses adds for this call
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Theoses's system prompt string rather than the final serialized provider payload.
...
```typescript
theoses.on("model_select", async (event, ctx) => {
...
```typescript
theoses.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `theoses.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, theoses waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "theoses-coding-agent";
...
```typescript
import { isBashToolResult } from "theoses-coding-agent";

theoses.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "theoses-coding-agent";

theoses.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap theoses's built-in local bash backend
...
```typescript
theoses.on("input", async (event, ctx) => {
...
Access to models, providers, and resolved authentication. `ctx.modelRegistry.getProvider(id)` returns the effective theoses-ai provider, while `getProviderAuth(id)` resolves its current API key, headers, base URL, and provider-scoped environment without requiring a loaded model. `ctx.model` is the active model, and `ctx.thinkingLevel` is its current effective thinking level.
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while theoses is idle.
...
Control flow helpers. `ctx.isIdle()` is false while Theoses is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of theoses.
...
```typescript
theoses.on("tool_call", (event, ctx) => {
...
Returns Theoses's current system prompt string.
...
```typescript
theoses.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Theoses currently uses to build the system prompt.
...
```typescript
theoses.registerCommand("my-cmd", {
  handler: async (args, ctx) => {
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `theoses` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
theoses.registerCommand("switch", {
...
- Captured old `theoses` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
theoses.registerCommand("handoff", {
...
```typescript
theoses.registerCommand("reload-runtime", {
...
export default function (theoses: ExtensionAPI) {
  ...
  theoses.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      theoses.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### theoses.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### theoses.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`theoses.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `theoses.getAllTools()` and are callable by the LLM without `/reload`.

Use `theoses.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
### theoses.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`theoses.appendEntry()`](#piappendentrycustomtype-data) with [`theoses.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### theoses.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
theoses.sendUserMessage("What is 2+2?");

// With content array (text + images)
theoses.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
theoses.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
theoses.sendUserMessage("And then summarize", { deliverAs: "followUp" });

// Opt in to extension command dispatch and skill/prompt template expansion
theoses.sendUserMessage("/review src/index.ts", { expandPromptTemplates: true });
```
...
### theoses.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `theoses.registerEntryRenderer()`.

```typescript
theoses.appendEntry("my-state", { count: 42 });
theoses.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### theoses.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
theoses.setSessionName("Refactor auth module");
```

### theoses.getSessionName()

Get the current session name, if set.

```typescript
const name = theoses.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### theoses.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
theoses.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
theoses.setLabel(entryId, undefined);
...
### theoses.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, theoses keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "theoses-tui";

theoses.registerCommand("deploy", {
...
### theoses.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `theoses.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### theoses.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, Theoses renders the transformed content with its built-in renderer.
...
```typescript
theoses.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
  if (isStreaming || messageType === "assistant-thinking") return markdown;
  return markdown.replaceAll("-->", "→");
});
```

If a transformer throws, Theoses keeps the Markdown produced so far and continues with the next transformer. The hook is display-only: the original message remains unchanged in the session and model context. It runs for new user messages, assistant streaming updates, restored session messages, and terminal width changes, so transformers should remain synchronous and inexpensive.

### theoses.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `theoses.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "theoses-tui";

theoses.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### theoses.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
theoses.registerShortcut("ctrl+shift+p", {
...
### theoses.registerFlag(name, options)

Register a CLI flag.

```typescript
theoses.registerFlag("plan", {
...
// Check value
if (theoses.getFlag("plan")) {
  // Plan mode enabled
}
```

### theoses.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await theoses.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### theoses.getActiveTools() / theoses.getAllTools() / theoses.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `theoses.getActiveTools()` returns the active tool names as `string[]`; `theoses.getAllTools()` returns metadata for all configured tools.

```typescript
const active = theoses.getActiveTools(); // ["read", "bash", ...]
const all = theoses.getAllTools();
// all = [{
//   name: "read",
...
theoses.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
theoses.setActiveTools(["read", "bash"]); // Switch to read-only
```

`theoses.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await theoses.setModel(model);
  if (!success) {
...
### theoses.getThinkingLevel() / theoses.setThinkingLevel(level)
...
```typescript
const current = theoses.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
theoses.setThinkingLevel("high");
```
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `theoses-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "theoses-ai";
...
theoses.registerProvider(provider);

// Register a new provider with custom models
theoses.registerProvider("my-proxy", {
  name: "My Proxy",
...
theoses.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
theoses.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
theoses.registerProvider("corporate-ai", {
...
The object form accepts a complete theoses-ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
### theoses.unregisterProvider(name)
...
```typescript
theoses.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    theoses.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `theoses.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `theoses.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "theoses-coding-agent";
...
theoses.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via theoses.exec (captured from extension closure)
    const result = await theoses.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Theoses persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `theoses-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when theoses resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
theoses -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
theoses --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [powershell.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/core/tools/powershell.ts) - `PowerShellToolDetails`
- [edit.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "theoses-coding-agent";
...
For `user_bash`, extensions can reuse theoses's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
export default function (theoses: ExtensionAPI) {
  ...
  theoses.registerTool({ name: "db_connect", ... });
  theoses.registerTool({ name: "db_query", ... });
  theoses.registerTool({ name: "db_close", ... });

  theoses.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/H4fizWasabie/theoses2/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "theoses-coding-agent";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `theoses.setActiveTools()` during execution. Theoses detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `theoses.registerTool()` so it appears in `theoses.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `theoses.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. Theoses records which tools were added on the loader's tool result.
5. Before the next model response, Theoses exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `theoses.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Theoses adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Theoses sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Theoses also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (theoses: ExtensionAPI) {
  ...
  theoses.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = theoses.getAllTools()
      ...
      const active = theoses.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      theoses.setActiveTools([...new Set([...active, ...added])]);
  ...
  theoses.on("session_start", () => {
    ...
    const initialTools = theoses.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    theoses.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("theoses - my-project");
...
```typescript
import { Text, Component } from "theoses-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "theoses-coding-agent";
import { matchesKey } from "theoses-tui";
...
theoses.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `theoses.sendMessage()`:
...
```typescript
theoses.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

theoses.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "theoses-coding-agent";
...
| `event-bus.ts` | Inter-extension events | `theoses.events` |
| **Session Metadata** |||
