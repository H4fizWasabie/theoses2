...
Theoses runs with all permissions by default, but in some cases, you will want to have more control over what directories Theoses can write to and which accesses it has.
...
Run the whole `theoses` process in Docker when you want the simplest local container boundary.
...
RUN npm install -g --ignore-scripts theoses-coding-agent
...
```bash
docker build -t theoses-sandbox -f Dockerfile.theoses .

docker run --rm -it \
  -e ANTHROPIC_API_KEY \
  -v "$PWD:/workspace" \
  -v theoses-agent-home:/root/.theoses/agent \
  theoses-sandbox
```
...
Launch `theoses` inside an OpenShell sandbox:

```bash
openshell sandbox create --name theoses-sandbox --from theoses -- theoses
```

In this pattern, the whole `theoses` process runs inside the sandbox.
...
```bash
openshell sandbox upload theoses-sandbox ./repo /workspace
openshell sandbox download theoses-sandbox /workspace/repo ./repo-out
```

OpenShell providers can keep raw model API keys outside the sandbox.
When inference routing is configured, code inside the sandbox can call `https://inference.local`, and the gateway injects the configured provider credentials upstream.
Configure Theoses to use the corresponding OpenAI-compatible or Anthropic-compatible endpoint if you want model traffic to use this route.
