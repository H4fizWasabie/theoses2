...
Set `THEOSES_SKIP_VERSION_CHECK=1` to disable the Pi version update check. Use `--offline` or `THEOSES_OFFLINE=1` to disable all startup network operations described here, including update checks and update telemetry.
