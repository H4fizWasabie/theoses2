...
Theoses uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.theoses/agent/settings.json` | Global (all projects) |
| `.theoses/settings.json` | Project (current directory) |
...
`theoses update` refreshes provider model catalogs.
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`THEOSES_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so theoses resumes after the editor exits:
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Theoses sees them, which may block the agent until the provider quota resets in some circumstances.
...
```json
{ "sessionDir": ".theoses/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `THEOSES_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.theoses/agent/settings.json` resolve relative to `~/.theoses/agent`. Paths in `.theoses/settings.json` resolve relative to `.theoses`. Absolute paths and `~` are supported.
...
Project settings (`.theoses/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.theoses/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .theoses/settings.json (project)
{
