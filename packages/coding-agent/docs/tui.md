> theoses can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`theoses-tui`](https://github.com/H4fizWasabie/theoses2/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "theoses-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `THEOSES_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "theoses-tui";
...
Import from `theoses-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "theoses-tui";
```
...
```typescript
import { matchesKey, Key } from "theoses-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "theoses-tui";
...
```typescript
import { getMarkdownTheme } from "theoses-coding-agent";
import { Markdown } from "theoses-tui";
...
Set `THEOSES_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
THEOSES_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `theoses-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "theoses-coding-agent";
import { DynamicBorder } from "theoses-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "theoses-tui";
...
```typescript
import { BorderedLoader } from "theoses-coding-agent";

theoses.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `theoses-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "theoses-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "theoses-tui";

theoses.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while theoses is streaming a response.
...
// Restore theoses's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "theoses-tui";
