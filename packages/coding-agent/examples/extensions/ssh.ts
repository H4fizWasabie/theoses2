...
export default function (theoses: ExtensionAPI) {
	theoses.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	theoses.on("session_start", async (_event, ctx) => {
		...
		const arg = theoses.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
