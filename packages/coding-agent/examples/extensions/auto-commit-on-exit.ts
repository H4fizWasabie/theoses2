...
export default function (theoses: ExtensionAPI) {
	theoses.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await theoses.exec("git", ["status", "--porcelain"]);
		...
		// Stage and commit
		await theoses.exec("git", ["add", "-A"]);
		const { code: commitCode } = await theoses.exec("git", ["commit", "-m", commitMessage]);
