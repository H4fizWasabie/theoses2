...
export default function (theoses: ExtensionAPI) {
	...
	theoses.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			theoses.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	theoses.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				theoses.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	theoses.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				theoses.sendUserMessage(args, { deliverAs: "followUp" });
	...
	theoses.registerCommand("askwith", {
