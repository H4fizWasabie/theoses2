...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "theoses-coding-agent";

export default function (theoses: ExtensionAPI) {
	theoses.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
