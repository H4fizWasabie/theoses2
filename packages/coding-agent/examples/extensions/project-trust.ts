 ...
 *
 *   mkdir -p ~/.theoses/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.theoses/agent/extensions/
 *
 * Or:
 *
...
import type { ExtensionAPI, ProjectTrustEventResult } from "theoses-coding-agent";
...
export default function (theoses: ExtensionAPI) {
	...
	theoses.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
