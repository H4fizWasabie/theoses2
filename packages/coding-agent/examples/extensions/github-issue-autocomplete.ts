...
async function resolveGitHubRepo(theoses: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await theoses.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (theoses: ExtensionAPI): void {
	theoses.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(theoses, ctx.cwd);
		if (!resolvedRepo.ok) {
