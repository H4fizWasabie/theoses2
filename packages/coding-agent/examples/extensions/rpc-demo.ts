...
export default function (theoses: ExtensionAPI) {
	...
	theoses.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	theoses.on("turn_end", async (_event, ctx) => {
	...
	theoses.registerCommand("rpc-input", {
	...
	theoses.registerCommand("rpc-editor", {
	...
	theoses.registerCommand("rpc-prefill", {
