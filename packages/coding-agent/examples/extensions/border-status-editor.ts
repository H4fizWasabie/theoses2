import { CustomEditor, type ExtensionAPI, type ExtensionContext, type KeybindingsManager } from "theoses-coding-agent";
import type { Component, EditorTheme, TUI } from "theoses-tui";
...
export default function (theoses: ExtensionAPI) {
	...
	theoses.on("agent_settled", () => {
		isWorking = false;
		stopSpinner();
		activeTui?.requestRender();
	});

	theoses.on("session_shutdown", () => {
		stopSpinner();
		activeTui = undefined;
	});
	...
	theoses.on("session_start", (_event, ctx) => {
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = theoses.getThinkingLevel();
