...
async function findConflicts(theoses: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await theoses.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (theoses: ExtensionAPI) {
	theoses.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await theoses.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await theoses.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await theoses.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await theoses.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await theoses.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await theoses.exec("git", ["merge", "--no-ff", ref]);
			if (mergeCode === 0) return;
		}

		// Either we just merged with conflicts, or we were already in an unfinished merge
		const conflicts = await findConflicts(theoses, ctx.cwd);
		if (conflicts.length === 0) return;

		theoses.sendUserMessage(formatConflicts(ref, conflicts), { deliverAs: "followUp" });
	});
}
