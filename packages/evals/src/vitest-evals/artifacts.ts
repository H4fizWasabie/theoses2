...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "theoses-evals:session" && artifact.type !== "theoses-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "theoses-evals:session" ? "sessions" : "sources";
