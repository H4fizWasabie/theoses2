...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasTheosesDocs: boolean;
...
function createExtensionAuthoringHarness(name: string, transformSystemPrompt?: (defaultPrompt: string) => string) {
	return createTheosesCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			const extensionPath = join(session.sessionManager.getCwd(), ".theoses", "extensions", "hello.ts");
			...
			return {
				...
				systemPromptHasTheosesDocs: session.systemPrompt.includes("\nTheoses documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default Theoses system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default Theoses system prompt has no working-directory section.");
	return defaultPrompt.slice(0, cwdStart);
}

const ExtensionAuthoringJudge = createJudge<TheosesCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("theoses-coding-agent")) {
				failures.push("extension does not import the canonical theoses-coding-agent package");
			}
...
const extensionHarnessTable = evalHarnessTable("Theoses extension authoring system prompt", {
...
describe.for(extensionHarnessTable)("$name", ({ harness }) => {
	describeEval(
		"Theoses extension authoring system prompt",
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				const result = await run([
					{
						type: "prompt",
						content:
							"Create a Theoses extension with a hello tool that takes a name and returns a greeting. For example, passing Bob should return `Hello, Bob!`.",
					},
					{ type: "reload" },
					{
						type: "prompt",
				...
				if (result.output.extensionSource !== null) {
					...
					if (typeof runId !== "string") throw new Error("Theoses eval run did not record a run ID.");
				...
				expect(result.output.systemPromptHasTheosesDocs).toBe(expectsFullPrompt);
			});
		},
	);
});
