...
export class TheosesServer {
	readonly id: string;

	private readonly listeners: readonly TheosesServerListener[];
	...
	constructor(service: TheosesServerService, options: TheosesServerOptions) {
	...
	start(): Promise<this> {
		if (this.started) return Promise.reject(new Error("TheosesServer is already started"));
		if (this.startPromise) return Promise.reject(new Error("TheosesServer is already starting"));
		if (this.closing) return Promise.reject(new Error("TheosesServer is closing or closed"));
		this.startPromise = this.startInternal();
		return this.startPromise;
	}

	private async startInternal(): Promise<this> {
		const started: TheosesServerListener[] = [];
		try {
	...
	private toProtocolError(error: unknown): ProtocolError {
		...
		if (error instanceof TheosesServerError) {
...
function resolveOptions(options: TheosesServerOptions): { maxFrameLength: number; handshakeTimeoutMs: number } {
	if (!Array.isArray(options.listeners)) throw new TypeError("TheosesServer listeners must be an array");
	if (options.serverId === "") throw new TypeError("TheosesServer serverId must not be empty");
	const maxFrameLength = options.maxFrameLength ?? DEFAULT_MAX_FRAME_LENGTH;
	if (!Number.isSafeInteger(maxFrameLength) || maxFrameLength <= 0 || maxFrameLength > MAX_UINT32) {
		throw new TypeError(`TheosesServer maxFrameLength must be an integer between 1 and ${MAX_UINT32}`);
	}
	...
	if (
		...
		throw new TypeError(`TheosesServer handshakeTimeoutMs must be an integer between 1 and ${MAX_TIMER_DELAY_MS}`);
	}
