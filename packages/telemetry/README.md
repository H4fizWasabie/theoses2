...
Vendor-neutral telemetry contracts and typed schema utilities for theoses packages.
...
Applications can use the in-memory reference or provide an adapter for OpenTelemetry, Sentry, logs, or another backend. Theoses packages pass telemetry contexts explicitly and define their domain schemas separately.
...
- [Theoses Package Integration](#theoses-package-integration)
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from 'theoses-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from 'theoses-telemetry';
...
`theoses-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from 'theoses-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from 'theoses-agent-core';
...
## Theoses Package Integration

Package ownership is intentionally split:

- `theoses-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `theoses-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `theoses-agent-core` owns and exports the theoses AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
...
The theoses schemas use theoses-owned `theoses.ai.*`, `theoses.harness.*`, and `theoses.session.*` names. Adapters may translate them to backend conventions without changing the emitted theoses vocabulary.
