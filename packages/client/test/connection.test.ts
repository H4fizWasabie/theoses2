...
import { type ByteTransportFactory, TheosesClient, TheosesDisconnectedError } from "../src/index.ts";
import {
	attachSession,
	baseServerSnapshot,
	connectClient,
...
describe("TheosesClient", () => {
	...
	test("does not restore a connection after a snapshot listener disconnects during handshake", async () => {
		...
		await expect(client.connect()).rejects.toBeInstanceOf(TheosesDisconnectedError);
		expect(client.connectionState).toBe("disconnected");
	...
	test("rejects pending requests on close and reconnects through a fresh factory result", async () => {
		...
		first.close();
		await expect(pending).rejects.toBeInstanceOf(TheosesDisconnectedError);
		expect(client.connectionState).toBe("disconnected");
	...
	test("rejects pending requests on transport errors", async () => {
		...
		await expect(pending).rejects.toMatchObject({ name: "TheosesDisconnectedError", message: "read failed" });
		expect(client.connectionState).toBe("disconnected");
	});
