...
export const VIEWPORT_TUI = Symbol.for("theoses-tui/viewport");
...
export abstract class TuiBase extends Container implements TUI {
	...
	private showHardwareCursor = process.env.THEOSES_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.THEOSES_CLEAR_ON_SHRINK === "1";
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string) {
		...
		this.logDirectory =
			logDirectory ?? process.env.THEOSES_CODING_AGENT_DIR ?? path.join(os.homedir(), ".theoses", "agent");
