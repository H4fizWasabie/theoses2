...
- Fixed debug and crash logs to use the configured TUI log directory, including `THEOSES_CODING_AGENT_DIR`, instead of always writing under `~/.theoses/agent` ([#6958](https://github.com/earendil-works/pi/pull/6958) by [@davidbrai](https://github.com/davidbrai)).
...
- Added `THEOSES_DEBUG_REDRAW=1` env var for debugging full redraws (logs triggers to `~/.theoses/agent/pi-debug.log`)

### Changed

- Terminal height changes no longer trigger full redraws, reducing flicker on resize
- `clearOnShrink` now defaults to `false` (use `THEOSES_CLEAR_ON_SHRINK=1` or `setClearOnShrink(true)` to enable)
...
- Added `THEOSES_TUI_WRITE_LOG` environment variable to capture raw ANSI output for debugging
