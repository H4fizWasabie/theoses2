...
function createModel(baseUrl: string): Model<"theoses-messages"> {
	return {
		id: "auto",
		name: "Radius Auto",
...
describe("theoses-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		...
		} satisfies TheosesMessagesOptions;
...
describe("theoses-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("theoses-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "theoses-messages";
		expect(api).toBe("theoses-messages");
	});
});
